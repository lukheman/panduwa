...
#[Title('Profile Bendahara - AdminPro')]
class Profile extends Component
{
    ...
    {
        $userId = Auth::guard('bendahara')->id();
        $rules = [
            'nama' => ['required', 'string', 'max:255'],
            'email' => ['required', 'email', 'max:255', 'unique:bendahara,email,' . $userId],
        ];

        if ($this->showPasswordSection && $this->password) {
            $rules['current_password'] = ['required', 'current_password:bendahara'];
    ...
    {
        $this->validate([
            'current_password' => ['required', 'current_password:bendahara'],
    ...
    {
        return view('livewire.bendahara.profile');
    }
}
