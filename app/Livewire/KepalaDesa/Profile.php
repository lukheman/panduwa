...
#[Title('Profile Kepala Desa - AdminPro')]
class Profile extends Component
{
    ...
    {
        $user = Auth::guard('kepala_desa')->user();
        if ($user) {
            $this->nama = $user->nama ?? '';
    ...
    {
        $userId = Auth::guard('kepala_desa')->id();
        $rules = [
            'nama' => ['required', 'string', 'max:255'],
            'email' => ['required', 'email', 'max:255', 'unique:kepala_desa,email,' . $userId],
        ];

        if ($this->showPasswordSection && $this->password) {
            $rules['current_password'] = ['required', 'current_password:kepala_desa'];
    ...
    {
        $this->validate([
            'current_password' => ['required', 'current_password:kepala_desa'],
    ...
    {
        return view('livewire.kepala-desa.profile');
    }
}
